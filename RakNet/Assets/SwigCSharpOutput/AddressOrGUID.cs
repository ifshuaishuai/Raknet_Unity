...
public class AddressOrGUID : global::System.IDisposable {
  ...
  public virtual void Dispose() {
    ...
    private static Dictionary<int, AddressOrGUID> _systemAddressDict = new Dictionary<int, AddressOrGUID>(1);
    private static Dictionary<int, AddressOrGUID> _guidDict = new Dictionary<int, AddressOrGUID>(1);

    public static implicit operator AddressOrGUID(SystemAddress systemAddress)
    {
        int hash = systemAddress.GetHashCode();
        AddressOrGUID result;
        if (_systemAddressDict.TryGetValue(hash, out result))
        {
            return result;
        }
        else
        {
            result = new AddressOrGUID(systemAddress);
            _systemAddressDict.Add(hash, result);
            return result;
        }
	} 
	...
	public static implicit operator AddressOrGUID(RakNetGUID guid)
	{
	    ...
	    AddressOrGUID result;
	    if (_guidDict.TryGetValue(hash, out result))
	    {
	        return result;
	    }
	    else
	    {
	        result = new AddressOrGUID(guid);
	        _guidDict.Add(hash, result);
